...
export const SpinnerLoadingIndicator = ({
  ...
  return (
    <div className="relative">
      <StatusBorder className={cn("border-blue-700/40", className)}>{children}</StatusBorder>
  ...
  return (
    <>
      ...
      `}
        ...
        <div className={cn("absolute inset-0 overflow-hidden rounded-[5px]",className)}>
...
const StatusBorder = ({
  ...
  return (
    <>
      <div
        className={cn(
          "absolute -top-px -left-px h-[calc(100%+2px)] w-[calc(100%+2px)] rounded-[5px] border-3",
          className,
        )}
      />
      {children}
...
export const NodeStatusIndicator = ({
  ...
  switch (status) {
    case "loading":
      switch (variant) {
        case "overlay":
          return <SpinnerLoadingIndicator className={className}>{children}</SpinnerLoadingIndicator>;
        case "border":
